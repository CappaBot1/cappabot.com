...
export async function run(req: Request): Promise<Response> {
    ...
    if (upgradeHeader.toLowerCase() === "websocket") {
        ...
        socket.onmessage = (event) => {
            ...
            if (json.message.length > maxMessageLength) {
    ...
    if (reqMethod == "POST") {
        ...
        try {
            ...
            if (parts.length >= 2) {
                ...
                responseMessage =
                    "Invalid message format. Use JSON or 'user: message'.";
            }
        }
        ...
        if (acceptHeader.includes("application/json")) {
            responseJSON = Array.from(messages).sort((a, b) =>
        ...
        return new Response("um idk", { status: 500 });
    }
}
